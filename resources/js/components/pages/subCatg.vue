<template>
	<div class="catg">
		...
		<div class="container" style="padding-top: 15px;">
			<div class="row">
				<div class="col-md-3" v-for="(c,index) in subCatgs" :key="index"  style="padding-top: 7px;">
					<div class="card">
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        ...
		        <div class="form-group">
				    ...
				    <input type="text" class="form-control" v-model="subCatg.title" id="title" aria-describedby="emailHelp" placeholder="الرجاء ادخال اسم الفئة">
				</div>
				  <div class="form-group">
				    <label for="catgs">اختيار الفئة الرئيسية</label>
				    <select class="form-control" id="catgs" v-model="subCatg.catg_id">
				      <option v-for="(c,index) in catgs" :value="c.id">{{c.title}}</option>
				    </select>
				  </div>
				<div class="form-group">
	...
	export default {
		...
		methods:{
			save() {
				const vm = this;
				vm.error = false;
				axios.post('/subCatg/store',vm.subCatg).then(response => {
					vm.subCatgs.push(response.data);
					vm.success = true;
					setTimeout(function() {
						$('#newCatg').modal('hide');
						vm.success = false;
						vm.subCatg = {title:'',image:''}
					}, 1000);
				}).catch(err => {
					vm.error = true;
				});
		    ...
		    createImage(file) {
		     let reader = new FileReader();
		      let vm = this;
		      reader.onload = (e) => {
		          vm.subCatg.image = e.target.result;
		      };
		      reader.readAsDataURL(file);
		   },
		   install() {
		   	const vm = this;
		   	axios.get('/subCatg/index').then(response => {
		   		vm.catgs = response.data.catgs;
		   		vm.subCatgs = response.data.subCatgs;
		   	}).catch(err => {
		   		console.log(err)
		   	});
		   },
		   destroy(id,index) {
		   	const vm = this;
		   	var index = index;
		   	confirm('هل تريد الحذف  ؟ ') && axios.get('/subCatg/destroy/' + id).then(response => {
		   		vm.subCatgs.splice(index,1);
		   	}).catch(err => {
		   		console.log(err)
		   	});
		   },
		   ...
		   onFileChangeU(e) {
			...
			updateImage() {
				const vm = this;
				axios.post('/subCatg/update/image/' + vm.editCatg.id, {image:vm.editCatg.image}).then(response => {
					vm.success = true;
				}).catch(err => {
					vm.error = true;
			...
			update() {
				const vm = this;
				vm.error = false;
				axios.post('/subCatg/update/title/' + vm.editCatg.id ,{title:vm.editCatg.title}).then(response => {
					vm.success = true;
					setTimeout(function() {
		   			$('#editCatg').modal('hide')
					vm.editCatg = null;
